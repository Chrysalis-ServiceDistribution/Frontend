...
export default function Authentication() {
  ...
  const handleSignUp = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    const username: string = (e.target as HTMLButtonElement).parentElement!.getElementsByClassName('Username')[0].getElementsByTagName('input')[0].value
    const email: string = (e.target as HTMLButtonElement).parentElement!.getElementsByClassName('Email')[0].getElementsByTagName('input')[0].value
    const password: string = (e.target as HTMLButtonElement).parentElement!.getElementsByClassName('Password')[0].getElementsByTagName('input')[0].value
    ...
    <> 
      <Flex direction='column' gap='3' justify='center' align='center'>
        ...
        <Flex direction='column-reverse'>
          {!signupMode &&(<Text size='1'>New? <Link size='1' onClick={() => {setSignupMode(!signupMode)}}> Sign-up</Link> Instead</Text>)}
          {signupMode &&(<Text size='1'> Already have an account? <Link size='1' onClick={() => {setSignupMode(!signupMode)}}> Login</Link>!</Text>)}
        </Flex>
      </Flex>
    </>
  )
